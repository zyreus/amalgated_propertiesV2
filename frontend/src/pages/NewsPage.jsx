    ...
    <div className="bg-brand-background-alt">
      ...
      <section className="py-16 sm:py-24">
        <div className="mx-auto grid max-w-7xl gap-6 px-4 sm:px-6 md:grid-cols-3">
          {news.map((item) => (
            <article key={item.id || item.title} className="rounded-3xl bg-white p-6 shadow-card">
        ...
        {loading && (
          <p className="mt-6 text-center text-sm text-brand-text/60">Loading latest news...</p>
        )}
      </section>
    </div>
  );
